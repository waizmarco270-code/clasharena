...
export function Tournament1v1Room({ tournament, tournamentId }: { tournament: any, tournamentId: string }) {
  const challengeId = tournamentId;
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(collection(db, 'tournaments', tournamentId, 'registrations'), (snap) => {
      const regs = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a: any, b: any) => {
        const timeA = a.registeredAt?.toMillis?.() || new Date(a.registeredAt).getTime() || 0;
        const timeB = b.registeredAt?.toMillis?.() || new Date(b.registeredAt).getTime() || 0;
        return timeA - timeB;
      });
  ...
  useEffect(() => {
    if (tournament) {
      setChallenge({ id: tournamentId, ...tournament });
      setLoading(false);
    }
  ...
  const handleClaim = async (claim: 'win' | 'loss') => {
    setIsClaiming(true);
    try {
      const res = await fetch('/api/arena/1v1/settle', {
        method: 'POST',
  ...
  const p1 = registrations[0] || null;
  const p2 = registrations[1] || null;
  
  if (challenge) {
    challenge.creatorId = p1?.id || challenge.creatorId;
    challenge.creatorName = p1?.username || p1?.name || 'Player 1';
    challenge.creatorAvatar = p1?.avatarUrl || challenge.creatorAvatar;
    challenge.acceptorId = p2?.id || challenge.acceptorId;
    challenge.acceptorName = p2?.username || p2?.name || 'Player 2';
    challenge.acceptorAvatar = p2?.avatarUrl || challenge.acceptorAvatar;
    challenge.pool = challenge.prizePool || challenge.pool;
    challenge.reqTh = challenge.townHall || challenge.reqTh;
  }
  ...
  const isSpectator = !isCreator && !isAcceptor && !isAdmin;
  ...
  const handleClanSetup = async () => {
     ...
     try {
       const res = await fetch('/api/arena/1v1/setup', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const submitRound = async (roundId: string) => {
    ...
    try {
      ...
      const res = await fetch('/api/arena/1v1/round/submit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          challengeId,
  ...
  const handleAdminAction = async (action: string, winnerId: string | null) => {
    ...
    try {
      const res = await fetch('/api/admin/arenahub/action-1v1', {
         method: 'POST',
         headers: { 'Content-Type': 'application/json' },
    ...
    <PageWrapper>
      <div className="relative min-h-screen pb-20">
        ...
        <div className="relative z-10 max-w-6xl mx-auto space-y-6 px-4 pt-8">
          ...
          {/* Clan Setup Banner */}
          {!challenge.clanLink && (isCreator || isAcceptor) && !isHistoryMode && (
          ...
          {!challenge.clanLink && isSpectator && !isHistoryMode && (
             <div className="p-4 bg-orange-500/10 border border-orange-500/30 rounded-xl flex items-center justify-center gap-2 text-orange-500 font-black uppercase text-sm animate-pulse">
                <Loader2 className="w-4 h-4 animate-spin" /> Waiting for Players to setup the Clan...
             </div>
          )}
                  ...
                  <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                    ...
                    <div className="flex flex-col items-center gap-3 w-full md:w-1/3">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Player 1 (TH {challenge.reqTh})</p>
                    ...
                    <div className="flex flex-col items-center gap-3 w-full md:w-1/3">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Player 2 (TH {challenge.reqTh})</p>
