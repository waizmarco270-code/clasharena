    ...
    <PageWrapper>
      <div className="max-w-7xl mx-auto flex flex-col gap-6 pb-20">
        ...
        <Tabs defaultValue="fixtures" className="w-full">
          ...
          <TabsContent value="fixtures" className="mt-4 outline-none relative group">
            <Card className="glass border-white/5 h-[80vh] relative overflow-hidden bg-[#0a0a0a] rounded-[2.5rem] shadow-2xl">
               {/* Controls Bar */}
               <div className="absolute top-6 left-6 z-50 flex items-center gap-2 bg-black/60 backdrop-blur-xl border border-white/10 p-1.5 rounded-full shadow-2xl">
                 ...
                 <Button size="icon" variant="ghost" className="h-9 w-9 text-white hover:bg-white/10" onClick={() => setZoom(1)}><Maximize2 className="w-4 h-4" /></Button>
               </div>
               ...
               <div className="absolute top-6 right-6 z-50">
                 <Badge variant="outline" className="bg-primary/20 border-primary/40 text-primary font-black uppercase tracking-widest px-4 py-2 flex items-center gap-2 shadow-xl backdrop-blur-md">
                   <Move className="w-4 h-4 animate-pulse" /> Movable Canvas
                 </Badge>
               </div>
                    ...
                    <div className="flex gap-32 items-center relative z-10">
                            ...
                            {roundMatches.map((m: any, mIdx: number) => (
                              <div key={m.id || mIdx} className="relative flex items-center">
                                    ...
                                    return (
                                      <div 
                                        ...
                                        className={cn(
                                          ...
                                          isAdmin && pId && pId !== 'bye' && !t?.status?.includes('completed') && !demoSize ? "cursor-pointer hover:border-primary/50" : "cursor-default",
                                      ...
                                      >
                                        <div className="flex items-center gap-3 overflow-hidden">
                                          <div className={cn("w-1 h-6 rounded-full", isWinner ? "bg-white" : "bg-primary/40")} />
                                          <span className="text-sm font-black uppercase truncate text-white">{pName || 'TBD'}</span>
                                        </div>
                                        {isWinner && <Crown className="w-4 h-4 text-white" />}
                                      </div>
                                    );
                                  })}
                                </div>
                                ...
                                {roundNum < totalRounds && (
                                  <svg className="absolute left-full top-1/2 -translate-y-1/2 w-32 h-[400px] pointer-events-none overflow-visible">
                                    <path 
                                      d={`M 0 200 L 32 200 L 32 ${mIdx % 2 === 0 ? 300 : 100} L 64 ${mIdx % 2 === 0 ? 300 : 100}`}
                                      fill="none" 
                                      stroke="currentColor" 
                                      strokeWidth="2"
                                ...
                                {/* Trophy Connection */}
                                {roundNum === totalRounds && (
                                  <div className="absolute left-full top-1/2 -translate-y-1/2 flex items-center gap-6 ml-12">
                                     <div className="w-12 h-0.5 bg-primary/20" />
                                     <div className={cn(
                                       "h-32 w-32 rounded-3xl border-4 flex flex-col items-center justify-center bg-black transition-all duration-700",
                                       winnerInfo ? "border-green-500 shadow-[0_0_60px_rgba(34,197,94,0.4)]" : "border-yellow-500/20 grayscale animate-pulse"
                                     )}>
                                        <Crown className={cn("w-12 h-12 mb-2", winnerInfo ? "text-green-500" : "text-yellow-500/20")} />
                                        <p className="text-[10px] font-black uppercase text-white/40 tracking-widest">{winnerInfo ? 'CHAMPION' : 'FINAL'}</p>
                                        <p className="text-xs font-black uppercase text-white truncate max-w-[100px] mt-1">{winnerInfo?.name || 'AWAITING'}</p>
                                     </div>
                                  </div>
                                )}
                              </div>
                      ...
                      {matches.length === 0 && (
                        <div className="flex flex-col items-center justify-center w-[80vw] py-20 text-center gap-6">
                           ...
                           <p className="text-sm font-black uppercase tracking-[0.5em] text-white/20">Awaiting Command Initialization</p>
                           {isAdmin && <Button onClick={generateFixtures} className="bg-primary px-10 h-14 rounded-2xl font-black uppercase">Start Protocol</Button>}
                        </div>
                      )}
                    </div>
                 </div>
          ...
          <TabsContent value="protocol" className="mt-4 outline-none">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="glass border-white/5 rounded-[2rem] overflow-hidden bg-black/40 p-8 space-y-8">
                  <h3 className="font-headline text-xl font-black uppercase italic tracking-tighter flex items-center gap-3"><LinkIcon className="text-primary" /> Clan Protocol</h3>
                  <div className="bg-black/60 rounded-2xl p-6 border border-white/5 space-y-6">
                     <div className="flex justify-between items-center border-b border-white/5 pb-4">
                       <span className="text-[10px] font-black uppercase text-muted-foreground">Protocol Tag</span>
                       <span className="text-lg font-black text-primary uppercase">{t?.clanUid || 'PENDING'}</span>
                     </div>
                     {t?.clanLink && (
                       <Button asChild className="w-full h-14 bg-green-600 font-black uppercase rounded-xl">
                          <a href={t.clanLink} target="_blank">JOIN WAR CLAN <ArrowRight className="ml-2 w-4 h-4" /></a>
                       </Button>
                     )}
                  </div>
                </Card>
                ...
                <Card className="glass border-white/5 rounded-[2rem] bg-black/40 p-8 space-y-8">
                   <h3 className="font-headline text-xl font-black uppercase italic tracking-tighter flex items-center gap-3"><ShieldAlert className="text-primary" /> Arena Protocols</h3>
...
function DropdownDemo({ onSelect, current }: { onSelect: (size: number) => void, current: number | null }) {
  return (
    <div className="flex items-center gap-2 mr-4 bg-muted/20 px-4 py-1 rounded-xl border border-white/10 shrink-0">
      ...
      <span className="text-[10px] font-black uppercase text-muted-foreground mr-2">DEMO:</span>
